...
def train(net, dataloader_train, dataloader_val, criterion, optimizer):
	...
	for e in range(cfg.train_number_epochs):
		...
		total = 0
		correct = 0
		for b, (img0, img1, label) in enumerate(dataloader_train):
			if torch.cuda.is_available() and len(cfg.gpu_device) >= 1:
				img0 = img0.cuda()
				img1 = img1.cuda()
				label = label.cuda()
		...
		if test_acc > best_acc:
			...
			torch.save(state, os.path.join(cfg.base_path, cfg.base_name))
		print()
...
if __name__ == "__main__":
	...
	if True:
		print("\nStart train !!!")
		cfg.base_name = cfg.network + "_" + time.strftime("%Y-%m-%d-%H_%M_%S.pth", time.localtime())
		print("Now time base_name: ", cfg.base_name, "\n")
